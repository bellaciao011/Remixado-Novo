...
const FROM_ADDRESS = 'Panini FIFA World Cup 2026 <onboarding@resend.dev>';
...
export async function sendUpsellConfirmation(
  ...
  try {
    const resend = getResendClient();
    const { subject, html } = buildUpsellConfirmationEmail(order, upsellProductName, upsellAmount);
    await resend.emails.send({
      from: FROM_ADDRESS,
      to: order.customerEmail,
      subject,
    ...
    console.log(`[email] Upsell confirmation sent to ${order.customerEmail}`);
  } catch (err) {
...
export async function scheduleLogisticsSequence(order: OrderInfo): Promise<void> {
  ...
  const results = await Promise.allSettled(
    LOGISTICS_DAYS.map(async (days, idx) => {
      ...
      const { subject, html } = buildLogisticsEmail(order, step);
      await resend.emails.send({
        from: FROM_ADDRESS,
        to: order.customerEmail,
        subject,
...
export async function sendTestEmail(templateNumber: number, toEmail: string): Promise<void> {
  ...
  if (templateNumber === 1) {
    ...
    const { subject, html } = buildUpsellConfirmationEmail(order, 'Caixa com 50 Envelopes', 16);
    await resend.emails.send({ from: FROM_ADDRESS, to: toEmail, subject, html });
  } else {
