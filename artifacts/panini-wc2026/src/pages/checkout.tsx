...
function PaymentForm({ amountTotal, shipping, items, orderBump1Selected, paymentIntentId }: PaymentFormProps) {
...
export default function CheckoutPage() {
  ...
  const handleToggleBump1 = async () => {
    if (!paymentIntentId || isUpdatingBump1) return;
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/api/checkout/payment-intent/${paymentIntentId}`, {
        ...
        body: JSON.stringify({ addOrderBump1: next1 }),
      });
      if (res.ok) {
